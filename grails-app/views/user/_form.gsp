<%@ page import="stricto_grails_exercicio.User" %>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'profile', 'error')} ">
	...
	<g:select id="profile" name="profile.id" from="${stricto_grails_exercicio.Profile.list()}" optionKey="id" value="${userInstance?.profile?.id}" class="many-to-one" noSelection="['null': '']"/>
...
<div class="fieldcontain ${hasErrors(bean: userInstance, field: 'following', 'error')} ">
	...
	<g:select name="following" from="${stricto_grails_exercicio.User.list()}" multiple="multiple" optionKey="id" size="5" value="${userInstance?.following*.id}" class="many-to-many"/>
